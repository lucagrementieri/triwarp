...
---------------------
Every ``@wp.kernel`` decoration parses its function and builds an ``Adjoint`` at *import* time, and
this package has 553 of them plus ~110 more generated by ``kernels/reduce.py``'s factories. Eagerly
importing the 51 public modules therefore pulled in all 56 kernel modules and cost **0.60 s** on top
of ``import warp``'s 0.21 s (measured, RTX 5090 box, ``-X importtime``: ``kernels.reduce`` 85.6 ms
of self time, ``kernels.algorithms.ball_pivoting`` 73.3, ``kernels.intersection`` 68.7,
``kernels.remesh`` 34.2, ``kernels.creation`` 32.9 -- decoration, essentially all of it).

**And it was not avoidable by importing a submodule instead**, which is the part worth knowing:
Python imports a parent package before its child, so ``import triwarp.edges`` ran this file and
measured the same **0.586 s** while ``triwarp/edges.py``'s own imports are three lines. This file
was the whole cost.

It is also what made ``import triwarp as tw`` -- which 45 of the 52 wrappers open with -- a
whole-package pull. Under lazy resolution that statement is nearly free and ``tw.laplacian`` costs
one ``__getattr__`` on first touch, after which it is a plain global lookup because the resolved
module is cached into this module's namespace.

Two things this deliberately does not change. Nothing is imported at a *different* time relative to
its own kernel module, so ``_register_overloads()`` still runs before the first launch through the
module that needs it (CLAUDE.md section 4) -- a wrapper cannot be reached without importing it. And
``wp.load_module`` / ``wp.force_load`` are still not called here: registration is not compilation,
and eager loading at import is the thing section 4 forbids.
"""
