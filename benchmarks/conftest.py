...
``--bench-all-libs``
    ...
    library that was already measured losing to triwarp by more than 2x, and ranking 3rd-or-worse
    overall for that exact cell, is skipped rather than re-timed (``skip_known_slow``, below) —
    round 10 measured **43.4 of the suite's 50.4 minutes** of pytest-benchmark's own timed regions
    as reference-library rows, most of it adding no information (a library already 10-1000x slower
    does not become more informative on the next mesh size), and this table's policy alone recovers
    31.7 of those minutes. Pass this flag before regenerating the table with
...
def _known_slow_libraries() -> dict[
    ...
    ``--benchmark-json`` output — see that script's docstring for the policy and the measured
    minutes it recovers. Missing or unreadable is treated as "skip nothing", not an error: the
