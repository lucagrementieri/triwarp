...
def test_submesh_from_face_indices_empty(device: str) -> None:
    ...
    faces_wp = wp.array(
        np.array([0, 1, 2, 0, 2, 3], dtype=np.int32), dtype=wp.int32, device=device
    )
...
def test_submesh_from_face_indices_duplicated(request: pytest.FixtureRequest) -> None:
    ...
    face_indices = wp.array(
        face_indices_np, dtype=wp.int32, device=mesh_wp.points.device
    )
    submesh_tm = tm.util.submesh(
        mesh_tm, [face_indices_np], repair=False, append=False
    )[0]
    submesh_vertices_wp, submesh_faces_wp = tw.selection.submesh_from_face_indices(
...
def test_face_indices_from_vertex_indices(
    request: pytest.FixtureRequest, face_mode: str
) -> None:
    mesh_tm, mesh_wp = request.getfixturevalue("icosahedron")
    rng = np.random.default_rng(11)
    n_vertices = mesh_tm.vertices.shape[0]
    vertex_indices_np = rng.choice(
        n_vertices, size=max(1, n_vertices // 4), replace=False
    ).astype(np.int32)
    vertex_indices = wp.array(
        vertex_indices_np, dtype=wp.int32, device=mesh_wp.points.device
    )
...
def test_face_indices_from_vertex_indices_empty() -> None:
    faces_wp = wp.array(
        np.array([0, 1, 2], dtype=np.int32), dtype=wp.int32, device="cpu"
    )
    vertex_indices_wp = wp.empty(0, dtype=wp.int32, device="cpu")
    face_indices_wp = tw.selection.face_indices_from_vertex_indices(
        faces_wp, vertex_indices_wp
    )
...
def test_submesh_from_vertex_indices(
    ...
    n_vertices = mesh_tm.vertices.shape[0]
    vertex_indices_np = rng.choice(
        n_vertices, size=max(3, n_vertices // 5), replace=False
    ).astype(np.int32)
    vertex_indices = wp.array(
        vertex_indices_np, dtype=wp.int32, device=mesh_wp.points.device
    )
...
def test_submesh_from_vertex_mask(
    ...
    exp_vertices_wp, exp_faces_wp = tw.selection.submesh_from_vertex_indices(
        ...
        mesh_wp.indices,
        wp.array(
            selected.astype(np.int32), dtype=wp.int32, device=mesh_wp.points.device
        ),
        face_mode=face_mode,
    )
    assert np.allclose(got_vertices_wp.numpy(), exp_vertices_wp.numpy())
