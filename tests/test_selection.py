...
def test_face_indices_for_vertices(request: pytest.FixtureRequest, face_mode: str) -> None:
    mesh_tm, mesh_wp = request.getfixturevalue("icosahedron")
    ...
    face_indices_wp = tw.selection.face_indices_for_vertices(mesh_wp.indices, vertex_indices, face_mode=face_mode)
    face_indices_ref_np = _face_indices_for_vertices_np(mesh_tm.faces, vertex_indices_np, face_mode=face_mode)
    assert np.array_equal(face_indices_wp.numpy(), face_indices_ref_np)


def test_face_indices_for_vertices_empty() -> None:
    ...
    face_indices_wp = tw.selection.face_indices_for_vertices(faces_wp, vertex_indices_wp)
...
def test_submesh_from_vertex_indices(request: pytest.FixtureRequest, face_mode: str) -> None:
    ...
    face_indices_np = _face_indices_for_vertices_np(mesh_tm.faces, vertex_indices_np, face_mode=face_mode)
    submesh_tm = tm.util.submesh(mesh_tm, [face_indices_np], repair=False, append=False)[0]
...
def _face_indices_for_vertices_np(
    faces_np: np.ndarray,
